...
export default function CompanyProfileActions({ profileId, profileName }: Props) {
  ...
  useEffect(() => {
    ...
    ;(supabase.from('company_candidate_inquiries') as any)
      ...
      .limit(1)
      .maybeSingle()
      .then(({ data }: any) => setExistingInquiry(data || null))
  }, [isCompany, profileId, user])
  ...
  const proposeNextStep = async () => {
    ...
    setMessage('')
    setExistingInquiry({ id: data?.id, status: 'pendiente', archived_at: null })
  }
  ...
  return <div className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-4 space-y-3">
    ...
    <div className="space-y-2 rounded-xl border border-indigo-200 bg-white p-3"><p className="flex items-center gap-1.5 text-xs font-bold text-indigo-950"><Send size={14} /> Iniciar selección directa</p><p className="text-[11px] leading-relaxed text-indigo-900/75">La empresa coordina entrevistas y altas de proveedor por sus propios canales. LABURANTE no emite órdenes de compra ni reemplaza la aprobación interna.</p><div className="grid gap-2 sm:grid-cols-2"><select value={process} onChange={(e) => setProcess(e.target.value as 'entrevista' | 'contratacion')} className="rounded-lg border border-[var(--color-laburante-border)] px-2.5 py-2 text-xs"><option value="entrevista">Proponer entrevista</option><option value="contratacion">Conversar contratación</option></select><button type="button" onClick={proposeNextStep} className="rounded-lg bg-indigo-700 px-3 py-2 text-xs font-bold text-white">Enviar propuesta</button></div><textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Mensaje opcional: proyecto, rol y próximos pasos" rows={2} className="w-full rounded-lg border border-[var(--color-laburante-border)] px-2.5 py-2 text-xs" />{refreshAvailable && <button type="button" onClick={refreshProposal} className="w-full rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2 text-xs font-bold text-indigo-800">Refrescar propuesta pendiente</button>}{feedback && <p className="text-[11px] font-semibold text-indigo-800">{feedback}</p>}</div>
