...
export default function CompanyWorkspace() {
  ...
  const loadOpportunities = async () => {
    ...
    if (!inquiries.error) {
      ...
      const staleIds = (inquiries.data || []).filter((item: any) => item.status === 'pendiente' && !item.archived_at && new Date(item.created_at) < today).map((item: any) => item.id)
  ...
  const publishOpportunity = async (event: React.FormEvent) => {
    ...
    if (!isPaidCompany) return setMessage('Publicar y derivar oportunidades es una función del plan Pago. Activá el plan para usarla.')
  ...
  const archiveCandidateInquiry = async (inquiryId: string) => {
    ...
    const { error } = await (supabase.from('company_candidate_inquiries') as any).update({ archived_at: archivedAt, updated_at: archivedAt }).eq('id', inquiryId).eq('company_id', user.id)
    if (!error) setCandidateInquiries((items) => items.map((item) => item.id === inquiryId ? { ...item, archived_at: archivedAt } : item))
  }
  ...
  return <div className="container py-10 md:py-16 space-y-8">
    ...
    <section className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
      <form onSubmit={publishOpportunity} className="rounded-2xl border border-[var(--color-laburante-border)] bg-[var(--color-laburante-surface)] p-6 space-y-4">
        ...
        {!isPaidCompany && <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-950"><strong>Disponible en plan Pago · ARS $39.900/mes.</strong> La cuenta Gratis permite buscar, guardar perfiles y proponer entrevistas; el plan Pago suma publicar y derivar oportunidades entre Empresas. <a href={`https://wa.me/${SITE_CONFIG.officialWhatsApp}?text=${encodeURIComponent(`Hola LABURANTE, quiero activar el plan Pago de Empresa para ${companyName}.`)}`} target="_blank" rel="noopener noreferrer" className="font-bold underline">Solicitar activación</a></div>}
        ...
        <button disabled={loading || !isPaidCompany} className="btn-dark inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-bold disabled:opacity-60">{loading ? 'Publicando...' : isPaidCompany ? 'Publicar y derivar oportunidad' : 'Disponible en plan Pago'} <ArrowRight size={15} /></button>
      </form>
