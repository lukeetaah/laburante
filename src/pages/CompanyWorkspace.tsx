...
export default function CompanyWorkspace() {
  ...
  const loadOpportunities = async () => {
    ...
    const [own, shared] = await Promise.all([
      ...
      (supabase.from('company_opportunity_shares') as any).select('id, status, match_reason, created_at, company_opportunities(title, description, origin_platform, origin_note, localidad, provincia)').eq('recipient_company_id', user.id).order('created_at', { ascending: false }),
    ])
  ...
  const publishOpportunity = async (event: React.FormEvent) => {
    ...
    setTitle(''); setDescription(''); setOriginPlatform(''); setOriginNote(''); setLoading(false)
  ...
  return <div className="container py-10 md:py-16 space-y-8">
    ...
    <section className="grid gap-6 lg:grid-cols-[1.05fr_.95fr]">
      <form onSubmit={publishOpportunity} className="rounded-2xl border border-[var(--color-laburante-border)] bg-[var(--color-laburante-surface)] p-6 space-y-4">
        <div><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-indigo-700"><Send size={15} /> Derivar una búsqueda</p><h2 className="mt-2 font-heading text-xl font-bold">Que la oportunidad siga circulando</h2><p className="mt-1 text-xs leading-relaxed text-[var(--color-laburante-text-secondary)]">Publicá el trabajo y LABURANTE lo ofrece a empresas similares por zona.</p></div>
